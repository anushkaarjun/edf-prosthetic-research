...
def plot_spectral_analysis(spectral_info: Dict, ax: Optional[plt.Axes] = None):
    ...
    ax.axvspan(
        band_range[0], band_range[1], alpha=0.2, color="green",
...
def plot_classification_with_confidence(
    ...
    colors_list = ["green" if label == predicted_label else "gray" for label in label_names]
    bars = ax.barh(label_names, probabilities, color=colors_list)
    
    # Add confidence annotation
    ax.axvline(
        x=confidence, color="red", linestyle="--", linewidth=2,
        label=f"Confidence: {confidence:.2%}"
    )
    
    ax.set_xlabel("Probability")
    ax.set_xlim(0, 1)
    ax.set_title(f"Classification Result: {predicted_label}\nConfidence: {confidence:.2%}")
    ax.legend()
...
def visualize_motor_movement(
    predicted_label: str, confidence: Optional[float] = None, ax: Optional[plt.Axes] = None
):
