...
def load_csp_svm_model(model_path: str, csp_path: str = None):
    """Load CSP+SVM model from saved file."""
    import pickle
    ...
    # Check if it's the new format (dict with 'csp', 'svm', 'classes')
    if isinstance(model_data, dict) and 'csp' in model_data and 'svm' in model_data:
        csp = model_data['csp']
        svm = model_data['svm']
        classes = model_data.get('classes', None)
        return csp, svm, classes
    ...
    if csp_path:
        ...
        raise ValueError("For old format models, csp_path is required")


def load_eegnet_model(model_path: str, device='cpu', n_classes=4, n_channels=64, n_samples=126):
    """Load EEGNet (EEGMotorImageryNet) model from saved file."""
    # Import model class directly
    sys.path.insert(0, os.path.join(os.path.dirname(__file__), "src"))
    import importlib.util
    model_path_file = os.path.join(os.path.dirname(__file__), "src", "edf_ml_model", "model.py")
    spec = importlib.util.spec_from_file_location("model", model_path_file)
    model_module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(model_module)
    EEGMotorImageryNet = model_module.EEGMotorImageryNet
    
    # Create model with correct dimensions
    model = EEGMotorImageryNet(n_channels=n_channels, n_classes=n_classes, n_samples=n_samples)
...
def load_improved_eegnet_model(model_path: str, device='cpu', n_classes=4, n_channels=64, n_samples=126):
    """Load ImprovedEEGNet model from saved file."""
    # Import improved model class
    sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "src", "edf_ml_model"))
    import importlib.util
    improved_model_path = os.path.join(os.path.dirname(__file__), "..", "src", "edf_ml_model", "improved_model.py")
    spec = importlib.util.spec_from_file_location("improved_model", improved_model_path)
    improved_model_module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(improved_model_module)
    ImprovedEEGNet = improved_model_module.ImprovedEEGNet
    
    # Create model with correct dimensions
    model = ImprovedEEGNet(n_channels=n_channels, n_classes=n_classes, n_samples=n_samples)
    state_dict = torch.load(model_path, map_location=device)
    model.load_state_dict(state_dict)
    model.eval()
    model.to(device)
...
async def predict(request: EEGDataRequest):
    ...
    try:
        ...
        if model_type == 'csp_svm':
        ...
        elif model_type in ['eegnet', 'improved_eegnet']:
            # EEGNet or ImprovedEEGNet prediction (expects batch, channels, time)
            model = models['model']
            device = models['device']
            classes = models['classes']
            
            # processed_data should be (1, n_channels, n_samples)
            # Add batch dimension if needed
            if len(processed_data.shape) == 2:
                processed_data = processed_data[np.newaxis, :, :]  # (1, channels, time)
            
            # Normalize data to [-1, 1] to match training
            import importlib.util
            ...
            processed_data, _ = normalize_signal(processed_data, method="minmax")
            
            # Convert to tensor (shape: 1, channels, time)
...
async def load_model(request: LoadModelRequest):
    ...
    try:
        if request.model_type_param == 'csp_svm':
            # Try new format first (combined file), then old format (separate files)
            csp, svm, classes = load_csp_svm_model(request.model_path, request.csp_path)
            # Use classes from model if available, otherwise default
            if classes is None:
            ...
            # Default to 4 classes, 64 channels, 126 samples for newly trained model
            # Can be overridden by model config if needed
            n_classes = 4
            n_channels = 64
            n_samples = 126  # Actual shape from training was 126 samples
            model, device = load_eegnet_model(request.model_path, device, n_classes, n_channels, n_samples)
            models = {
                'model': model,
                'device': device,
                'classes': ['Both Feet', 'Both Fists', 'Left Hand', 'Right Hand']  # Order from training
            }
            model_type = 'eegnet'
        
        elif request.model_type_param == 'improved_eegnet':
            device = 'cuda' if torch.cuda.is_available() else 'cpu'
            # ImprovedEEGNet uses same dimensions as EEGNet
            n_classes = 4
            n_channels = 64
            n_samples = 126
            model, device = load_improved_eegnet_model(request.model_path, device, n_classes, n_channels, n_samples)
            models = {
                'model': model,
                'device': device,
                'classes': ['Both Feet', 'Both Fists', 'Left Hand', 'Right Hand']  # Same as EEGNet
            }
