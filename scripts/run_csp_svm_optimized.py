...
def main(base_path=None, use_lda=False, test_size=0.15):
    ...
    for subj, data in subject_data.items():
        ...
        print(f"Train: {X_train.shape[0]}, Test: {X_test.shape[0]} (test_size={test_size})")
        
        if use_lda:
            # Try LDA instead of SVM (often better for motor imagery)
            print("\nUsing LDA classifier (often better for motor imagery)...")
            csp = CSP(n_components=20, reg=None, log=True, norm_trace=False)  # More components for LDA
            ...
            print(f"  Searching {len(expanded_param_grid['csp__n_components'])} × "
            ...
            grid_search = GridSearchCV(
                pipeline, expanded_param_grid, cv=5,  # 5-fold CV instead of 3
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use-lda", action="store_true",
    ...
    try:
        test_acc, results = main(BASE_PATH, use_lda=args.use_lda, test_size=args.test_size)
