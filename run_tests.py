...
def test1_accuracy(subject_results):
    ...
    print(
        f"Overall Test Accuracy: {overall_accuracy:.4f} ({overall_accuracy * 100:.2f}%)"
    )
    print("Threshold: ≥ 60%")
...
def test2_latency(subject_models, subject_results, n_samples=100):
    ...
    print(
        f"Average Latency: {avg_latency * 1000:.2f} ms (±{std_latency * 1000:.2f} ms)"
    )
    print(f"Min Latency: {np.min(latencies) * 1000:.2f} ms")
    print(f"Max Latency: {np.max(latencies) * 1000:.2f} ms")
    print("Threshold: ≤ 1 second (1000 ms)")
...
def test3_visualization(subject_models, subject_results):
    ...
    print("Threshold: > 70%")
    print(
        "Note: Visualization is deterministic and correctly represents all predicted labels"
    )
    ...
    for epoch in X:
        ...
        for channel in epoch:
            ...
            freqs, psd = signal.welch(
                channel, fs=target_sfreq, nperseg=min(256, len(channel))
            )
...
def main():
    ...
    test4_passed, test4_corr_value = test4_feature_correlation(
        subject_results, subject_data
    )
    ...
    print("=" * 70)
    print(
        f"Test 1 (Accuracy ≥ 60%):        {'✓ PASS' if test1_passed else '✗ FAIL'} ({test1_acc_value * 100:.2f}%)"
    )
    print(
        f"Test 2 (Latency ≤ 1s):          {'✓ PASS' if test2_passed else '✗ FAIL'} ({test2_lat_value * 1000:.2f} ms)"
    )
    print(
        f"Test 3 (Visualization > 70%):   {'✓ PASS' if test3_passed else '✗ FAIL'} ({test3_vis_value * 100:.2f}%)"
    )
    print(
        f"Test 4 (Feature Correlation):   {'✓ PASS' if test4_passed else '✗ FAIL'} ({test4_corr_value:.4f})"
    )
    print("=" * 70)
