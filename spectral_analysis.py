...
try:
    from edf_ml_model.preprocessing import (
        ...
        FREQ_LOW, FREQ_HIGH, TARGET_SFREQ
    )
    FUNCTIONS_AVAILABLE = True
except ImportError:
    # Fallback if modules not available (e.g., missing dependencies)
    print("Warning: Could not import preprocessing functions. Using fallback implementations.")
...
def plot_spectral_comparison(raw_before: mne.io.Raw, raw_after: mne.io.Raw,
                             freq_low: float = FREQ_LOW, freq_high: float = FREQ_HIGH,
                             subject_id: str = "", save_path: str = None):
    """
    ...
    freqs_before, psd_before = get_psd_from_spectral_info(raw_before, freq_range=(1, 50))
    ...
    fig.suptitle(f'Spectral Analysis - Subject {subject_id}\n'
                 f'Bandpass Filter: {freq_low}-{freq_high} Hz', fontsize=14)
    
    # Plot 1: Before filtering (full range)
    ax1 = axes[0, 0]
    ax1.plot(freqs_before, psd_before_mean, 'b-', label='Mean PSD', linewidth=2)
    ax1.fill_between(freqs_before, psd_before_mean - psd_before_std,
                     psd_before_mean + psd_before_std, alpha=0.3, color='blue')
    ax1.axvspan(freq_low, freq_high, alpha=0.2, color='green', 
    ...
    ax2.fill_between(freqs_after, psd_after_mean - psd_after_std,
                     psd_after_mean + psd_after_std, alpha=0.3, color='red')
    ax2.axvspan(freq_low, freq_high, alpha=0.2, color='green',
                label=f'Motor Imagery Band ({freq_low}-{freq_high} Hz)')
    ...
    ax3.plot(freqs_before[mask_before], psd_before_mean[mask_before],
             'b-', label='Before Filtering', linewidth=2, alpha=0.7)
    ax3.plot(freqs_after[mask_after], psd_after_mean[mask_after],
             'r-', label='After Filtering', linewidth=2, alpha=0.7)
    ax3.axvspan(freq_low, freq_high, alpha=0.2, color='green',
                label=f'Motor Imagery Band ({freq_low}-{freq_high} Hz)')
    ...
    ax4.plot(freqs_before[mask_before_band], psd_before_mean[mask_before_band],
             'b-', label='Before Filtering', linewidth=2, alpha=0.7)
    ax4.plot(freqs_after[mask_after_band], psd_after_mean[mask_after_band],
             'r-', label='After Filtering', linewidth=2, alpha=0.7)
    ax4.axvspan(freq_low, freq_high, alpha=0.3, color='green',
                label=f'Motor Imagery Band ({freq_low}-{freq_high} Hz)')
...
def analyze_subject(subject_path: str, freq_low: float = FREQ_LOW,
                    freq_high: float = FREQ_HIGH, max_files: int = 3):
    """
    ...
    for file_idx, file_path in enumerate(edf_files):
        ...
        try:
            ...
            print(f"    Channels: {raw.info['nchan']}, Sampling rate: {raw.info['sfreq']:.1f} Hz")
            
            # Resample if needed
            if raw.info["sfreq"] != TARGET_SFREQ:
                print(f"    Resampling from {raw.info['sfreq']:.1f} Hz to {TARGET_SFREQ} Hz")
            ...
            print(f"      Before: {power_in_band_before:.2e} ({100*power_in_band_before/total_power_before:.1f}% of total)")
            print(f"      After:  {power_in_band_after:.2e} ({100*power_in_band_after/total_power_after:.1f}% of total)")
    ...
    if len(all_psd_before) > 0 and len(all_psd_after) > 0:
        ...
        plot_spectral_comparison(raw_before, raw_after, freq_low, freq_high,
                                subject_id=subject_id, save_path=save_path)
...
def main(base_path=None, freq_low=None, freq_high=None, max_subjects=3, max_files=3):
    ...
    if not os.path.exists(base_path):
        ...
        print(f"Please specify the correct path using --data-path")
        return
    ...
    print("  - Motor imagery signals are typically in the 8-30 Hz range (alpha + beta bands)")
    print("  - After filtering, power outside the band should be significantly reduced")
    print("  - If you see strong signals outside the motor imagery band, consider adjusting filter frequencies")
    print("\nFrequency Bands:")
