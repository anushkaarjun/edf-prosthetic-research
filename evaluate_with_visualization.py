...
from edf_ml_model.preprocessing import (
    preprocess_raw, detect_bad_data, clean_data, create_half_second_epochs,
    FREQ_LOW, FREQ_HIGH, TARGET_SFREQ, EPOCH_WINDOW
)
from edf_ml_model.data_utils import get_run_number, annotation_to_motion
from edf_ml_model.visualization import (
    plot_classification_with_confidence, visualize_motor_movement, plot_spectral_analysis
)
...
def load_and_evaluate_with_cleaning(subject_path: str, model_dict: dict, 
                                     clean_bad_data: bool = True) -> dict:
    """
        ...
        try:
            ...
            if clean_bad_data:
                ...
                if len(quality_info['bad_channels']) > 0:
                    print(f"  File {os.path.basename(file_path)}: Found {len(quality_info['bad_channels'])} bad channels")
    ...
    results = {
        ...
        'correct_mean_confidence': np.mean(correct_confidences) if len(correct_confidences) > 0 else 0.0,
        'incorrect_mean_confidence': np.mean(incorrect_confidences) if len(incorrect_confidences) > 0 else 0.0,
...
def plot_confusion_matrix_with_confidence(y_true, y_pred, confidences, 
                                         label_names, subject_id: str, 
                                         save_path: str = None):
    """
...
def plot_confidence_analysis(results: dict, save_path: str = None):
    ...
    axes[0, 0].axvline(results['mean_confidence'], color='r', linestyle='--', 
                      label=f'Mean: {results["mean_confidence"]:.3f}')
    axes[0, 0].axvline(results['median_confidence'], color='g', linestyle='--',
    ...
    for pred, true, conf in zip(results['predictions'], results['true_labels'], confidences):
    ...
    line1 = ax2.plot(thresholds, accuracies_at_threshold, 'b-o', label='Accuracy', linewidth=2)
    line2 = ax2_twin.plot(thresholds, n_samples_at_threshold, 'r-s', label='N Samples', linewidth=2)
...
def visualize_sample_predictions(results: dict, n_samples: int = 6, 
                                save_path: str = None):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-clean",
        action="store_true",
        help="Disable bad data cleaning"
    )
