...
data_utils_path = os.path.join(
    os.path.dirname(__file__), "src", "edf_ml_model", "data_utils.py"
)
...
preprocessing_path = os.path.join(
    os.path.dirname(__file__), "src", "edf_ml_model", "preprocessing.py"
)
...
try:
    model_path = os.path.join(
        os.path.dirname(__file__), "src", "edf_ml_model", "model.py"
    )
...
def load_and_preprocess_data(base_path, max_subjects=5):
    ...
    for subj in subjects:
        ...
        if len(X_list) > 0:
            ...
            print(
                f"  Subject {subj_id}: {X_subj.shape[0]} epochs, shape {X_subj.shape}"
            )
            print(
                f"    Normalized range: [{actual_min:.4f}, {actual_max:.4f}] (target: [-1, 1])"
            )
...
def main(base_path=None, max_subjects=5, freeze_after=30, epochs=50):
    ...
    print(
        f"Average Validation Accuracy: {overall_val_acc:.4f} ({overall_val_acc * 100:.2f}%)"
    )
    print(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data-path", type=str, default="./data", help="Path to EDF data directory"
    )
    parser.add_argument(
        "--max-subjects",
        type=int,
    ...
    parser.add_argument(
        "--freeze-after", type=int, default=30, help="Epoch to freeze backbone weights"
    )
    parser.add_argument(
        "--epochs", type=int, default=50, help="Total number of training epochs"
    )
