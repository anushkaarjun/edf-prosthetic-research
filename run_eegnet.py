...
warnings.filterwarnings(
    "ignore", category=RuntimeWarning, message=".*epochs were dropped.*"
)
...
class EEGNet(nn.Module):
    ...
    def __init__(self, n_classes, Chans=64, Samples=125, dropout_rate=0.25):
        ...
        self.conv3_sep = nn.Conv2d(
            32, 32, (1, 8), padding=(0, 4), groups=32, bias=False
        )
...
def main(base_path=None):
    ...
    for subj, data in subject_data.items():
        ...
        print(
            f"Model parameters: n_classes={n_classes}, n_channels={n_channels}, n_samples={n_samples}"
        )

        # Create model
        model = EEGNet(
            n_classes=n_classes, Chans=n_channels, Samples=n_samples, dropout_rate=0.5
        ).to(device)
    ...
    for subj, result in subject_results.items():
        ...
        print(
            classification_report(
                result["y_test"],
                result["y_pred"],
                target_names=result["unique_labels"],
                digits=3,
            )
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run EEGNet training and test accuracy"
    )
    parser.add_argument(
        "--data-path",
        type=str,
